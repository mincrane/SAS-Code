...
data new2;
set new end = last;
file 'risk_cat_daily.sas' dlm= ',' mod; 
if _n_ = 1 then do;
...
if last then do;
if &ind = 1 then do;
put  ' -11) and bbe.Trans_dt between &rundt. - ' "&day." ' and &rundt. THEN ITEM_PRICE_NUM  * ITEM_SOLD_QTY  * CAST(LSTG_CRNCY_EXCHNG_RATE AS DECIMAL(18,2)) else 0 end) as AMT_'  "&varname&day.D "  ');'; 
end;
if &ind = 2 then do;
put  ' -11) and bbe.Trans_dt between &rundt. - ' "&day." ' and &rundt. THEN 1 else 0 end) as CNT_'  "&varname.&day.D  "  ');'; 
end;
