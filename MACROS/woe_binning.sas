/*
%num_woe(data=imputed,y=ins,x=ddabal,output_folder=/some/path/to/output);
*/
...
* ======================================================= *;
* OUTPUTS:                                                *;
*  WOE: A FILE OF WOE TRANSFORMATION RECODING    		  *;
*  FMT: A FILE OF BINNING FORMAT                 		  *;
*  PUT: A FILE OF PUT STATEMENTS FOR *.FMT FILE  		  *;
*  SUM: A FILE WITH PREDICTABILITY SUMMARY       		  *;
*  OUT: A FILE WITH STATISTICAL DETAILS          		  *;
*  IMP: A FILE OF MISSING IMPUTATION RECODING    		  *;
* ======================================================= *;
***********************************************************;
 
options nocenter nonumber nodate mprint mlogic symbolgen orientation = landscape ls = 150;
...
* WOE RECODING FILE                     *;
filename woefile "&output_folder/WOE";
 
* FORMAT FOR BINNING                    *;
filename fmtfile "&output_folder/FMT";
 
* PUT STATEMENT TO USE FORMAT           *;
filename binfile "&output_folder/PUT";
 
* KS SUMMARY                            *;
filename sumfile "&output_folder/SUM";
  
* STATISTICAL SUMMARY FOR EACH VARIABLE *;
filename outfile "&output_folder/OUT";
 
* IMPUTE RECODING FILE                  *;
filename impfile "&output_folder/IMP";
    ...
    *** OUTPUT RECODING FILES IF IV >= &miniv BY DEFAULT ***;
    %if &ivflg = 1 %then %do;
      data _tmp5;
        ...
        %trim(&var) = compress(put(woe, 12.8), ' ');
      run;
