...
ods html file = 'prem_tpv_cutoff.xls';


%tier_sum(datain=prem,tier =  prempers_gtpv_5k , perf =perf_flag_180, bad = 1 ,tpv = perf_gtpv_d180 , loss = perf_gross_loss_d180,tierfmt=tpvfmt);
...
proc freq data=prem;
	table perf_flag_180 prempers_gtpv_5k*perf_flag_180/missing;
	format prempers_gtpv_5k display.;
run;


proc tabulate data = prem missing order=formatted;
	class  ap_type3 prempers_gtpv_5k;
	var perf_flag_180 perf_gtpv_d180 perf_gross_loss_d180 perf_net_loss_d180;
  table ap_type3=" "*(prempers_gtpv_5k='' all) all,
                      n="Total Num"*f=8.0 
                      perf_flag_180 ='Perf'*(n="Total"*f=8.0 sum='Bad'*f=8.0 mean='Bad Rate'*f=percent8.5) 
	                    perf_gtpv_d180='GTPV'*(sum*f=10.0 mean) 
	                    perf_gross_loss_d180='Gloss'*(sum*f=10.0 mean rowpctsum<perf_gtpv_d180>='bps'*f=5.2) 
	                    perf_net_loss_d180='Nloss'*(sum*f=10.0 mean rowpctsum<perf_gtpv_d180>='bps'*f=5.2)
	                    
	                    
	                    /Box="Acct Type" row=float RTS=25;
 	                    
	format prempers_gtpv_5k tpvfmt. ;                    
run;

/*** summary waterfall ***/

proc tabulate data = prem missing order=formatted;
	class  ap_type3 flag_existing;
	var perf_flag_180 perf_gtpv_d180 perf_gross_loss_d180 perf_net_loss_d180 score_con_scaled;
  table ap_type3=" "*(flag_existing='' all) all,
                      n="Total Num"*f=8.0 
                     	                    
	                    /Box="Acct Type" row=float RTS=25;

where prempers_gtpv_5k >=5000;
format flag_existing segfmt.;
run;
...
format flag_existing segfmt.;
where flag_match_stag = 1 and prempers_gtpv_5k >=5000;;
run;

proc tabulate data = prem missing order=formatted;
	class  ap_type3 prempers_gtpv_5k flag_existing;
	var perf_flag_180 perf_gtpv_d180 perf_gross_loss_d180 perf_net_loss_d180;
  table ap_type3=" "*(flag_existing='' all) all,
                      n="Total Num"*f=8.0 
                      perf_flag_180 ='Perf'*(n="Total"*f=8.0 sum='Bad'*f=8.0 mean='Bad Rate'*f=percent8.5) 
	...
	format prempers_gtpv_5k tpvfmt. ;   
	 where flag_match_stag = 1 and  perf_flag_180>=0 and prempers_gtpv_5k >=5000;                
run;
