  ...
  %ELSE %DO; /*Start performance check BLOCK*/
    ...
    */
    ...
    %IF %upcase(&sampling) = PROPORTIONAL %THEN %DO;
	    ...
	    * 10/21/2011 Sean: added the code for stratified sampling;
          proc sort data=&dset;
			by &perfvar;
          run;
	    ...
	    * 10/21/2011 Sean: added the code for stratified sampling;
        ...
        data lrdat.build lrdat.valid;
	    	...
	    	* 08/29/2012 Sean: Above code 'surveyselect' added the following two variables so need to drop them;
			drop SamplingWeight SelectionProb;
        run; 
    %END;
...
%IF %upcase(&finaliter.) = YES %THEN %DO;
	...
	%popstab(req=base,dataset=lrdat.scrtotal,_var=predicted,cutoff=0,cutoff_opt=0,path_base=&rootdir./Final_Stats/Population_Stability/,format_opt=yes,base_format_name=format);
	...
	%score_dist(dataset=lrdat.scrtotal,_var=predicted,breaks=20,bad_flag=AR_flag,fmtfilename=&rootdir./Final_Stats/Score_distribution/cuts_total.txt,gen_opt=yes);
	...
	%char_analysis(variable=&varlist_tot., data_set=lrdat.scrtotal,p_var_=AR_flag,path_base=&rootdir./Final_Stats/Char_analysis/,fmtfilename_=fl_&segment.,vargen_opt_=yes,require=base);
	...
	%score_dist(dataset=lrdat.scrbld,_var=predicted,breaks=20,bad_flag=AR_flag,fmtfilename=&rootdir./Final_Stats/Score_distribution/cuts_bld.txt,gen_opt=yes);
	...
	%IF %sysfunc(exist(lrdat.scrVLD)) = 1 %THEN %DO; 
		...
		%score_dist(dataset=lrdat.scrVLD,_var=P_1,breaks=20,bad_flag=AR_flag,fmtfilename=&rootdir./Final_Stats/Score_distribution/cuts_vld.txt,gen_opt=yes);
		
		data score_dist_vld;
			set score_dist1;
			label lo= "Score*Low End" hi= "Score*High End" no_of_acc= "Total # of*Accounts" event_acc= "# of Good*Accounts"
				  non_event_acc= "# of Bad*Accounts" percent_event= "% of Good*Accounts" percent_non_event= "% of Bad*Accounts" delta= "Difference*in %"
				  woe= "Weight of*Evidence" cum_non_event= "Cumulative % of*Bad Accounts" cum_event= "Cumulative % of*Good Accounts" iv= "Information*Value" ks_spread ="KS Spread";
		run;
		
	%END;
	
	data gini;
		merge score_dist_total(keep=cum_event cum_non_event rename=(cum_event=cum_event_total cum_non_event=cum_non_event_total)) score_dist_bld(keep=cum_event cum_non_event rename=(cum_event=cum_event_bld cum_non_event=cum_non_event_bld)) 
												%IF %sysfunc(exist(lrdat.scrVLD)) = 1 %THEN %DO;
													score_dist_vld (keep=cum_event cum_non_event rename=(cum_event=cum_event_vld cum_non_event=cum_non_event_vld))
												%END;
											  ;
	run;
...
/* *******************************************************************************************/
	...
	%IF &vldset ^= 0 %THEN %DO; 
		...
		%let _vldsamp_nonevent=%eval(&_vldsamp. - &_vldsamp_event.); 
		%let _wvldsamp_nonevent=%eval(&_wvldsamp. - &_wvldsamp_event.);
	%END;
	...
	%let totalsamp_nonevent=%eval(&totalsamp.- &totalsamp_event.);
	%let wtotalsamp_nonevent=%eval(&wtotalsamp.- &wtotalsamp_event.);
	%let _bldsamp_nonevent=%eval(&_bldsamp. - &_bldsamp_event.);
	%let _wbldsamp_nonevent=%eval(&_wbldsamp. - &_wbldsamp_event.);
	...
	proc sql;
		...
		insert into iter_summary
			values("Development","Weighted",&_bldsamp., &_bldsamp_event., &_bldsamp_nonevent.)
			%IF &vldset ^= 0 %THEN %DO;
				values("Validation","Weighted",&_vldsamp., &_vldsamp_event., &_vldsamp_nonevent.)
			%END;
			values("Total","Weighted",&totalsamp., &totalsamp_event., &totalsamp_nonevent.)
			values("Development","Unweighted",&_wbldsamp., &_wbldsamp_event., &_wbldsamp_nonevent.)
			%IF &vldset ^= 0 %THEN %DO;
				values("Validation","Unweighted",&_wvldsamp., &_wvldsamp_event., &_wvldsamp_nonevent.)
			%END;
			values("Total","Unweighted",&wtotalsamp., &wtotalsamp_event., &wtotalsamp_nonevent.);
	quit;
...
********************************************************************************************************************/	
	filename reports "Final_Stats/business_doc.html" mod;
	...
	ods html file=reports style=journal;
		...
		sum no_of_acc non_event_acc event_acc iv;
		run;
	...
	ods html file=reports style=journal;	
		...
		var lower_limit upper_limit base_cnt base_percent;
