...
/* loop until  the maximum number of 
...
/********
 ...
 Obtain the binning rules 
 Method: 1=gini 2=Entory 3=Pearson's Chi2 4=Information Value
 Acc=% of IV to be tested as the minimum bin size 

%BinContVar(&DSin, &IVVar, &DVVar, &Method, &MMax, &Acc, &DSVarMap);

 To apply the binning maps to the variable, use ApplyMap2: 
 
%ApplyMap2(&DSin, &IvVar, Newbin, &DSVarMap, DSout);
