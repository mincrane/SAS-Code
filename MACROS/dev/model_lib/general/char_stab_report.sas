...
* Parameters involved:
					require			  -- BASE/ CURR - depending on whether you are generating baselines or creating tracking reports
					variable		  -- list of variables involved seperated by a space.
					data_set          -- complete path of the dataset for which the baseline statistics are to be drawn.
					P_VAR             -- performance variable
					FMTFILENAME       --  Name of the file containing the decile ranges. Defaults to flag_logic
					path_base		  -- Path of where the formats should be stored.
	...
	*Creates a temporary dataset - work.char_stab_report_&i. USer has to print this dataset to view results
									&i. =1,2,3,4,5,.... which corrresponds to each variable in the list
